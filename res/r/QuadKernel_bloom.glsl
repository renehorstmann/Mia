...
void main() {
    ...
    for (int y=0; y<kernel_size.y; y++) {
        for (int x=0; x<kernel_size.x; x++) {
            ...
            // apply thresh; for example a mask of
            //     vec4(0.9, 2, 2, 0.9) will only "select" non opaque full red colors (mixed with red (like white), too)
            // threshed is either 0.0 or 1.0
            //vec4 diff = max(vec4(0.0), tex_color - u_tloom_tresh);
            //float threshed = min(1.0, (diff.r + diff.g + diff.b + diff.a) *1000.0);

            vec3 masked = tex_color.rgb * u_kernel_mask.rgb;

            float threshed = max(masked.r, max(masked.g, masked.b));
            threshed = min(1.0, max(0.0, threshed - u_kernel_mask.a) * 1000.0);

            // result color is max color in the kernel, where mask applies
            rgba = max(rgba, kernel * tex_color * threshed);
        }
    }
