#ifndef M_UTILS_FLT_CAMERA_H
#define M_UTILS_FLT_CAMERA_H
 ...
 */
O_INLINE
m_mat4 mat4_camera_ortho(float left, float right,
 ...
 */
O_INLINE
m_mat4 mat4_camera_frustum(float left, float right,
                           float bottom, float top,
                           float near_z, float far_z)
{
 ...
 */
O_INLINE
m_mat4 mat4_camera_perspective(float fovy, float aspect, float near_z, float far_z)
{
    // from cglm/cam.h/glm_perspective
    float f = (float) 1 / m_tan(fovy * (float) 0.5);
 ...
 */
O_INLINE
m_mat4 mat4_camera_lookat(m_vec3 eye, m_vec3 center, m_vec3 up)
{
    // from cglm/cam.h/glm_lookar

    m_vec3 f = vec3_sub_v(center, eye);
    f = vec3_normalize(f);

    m_vec3 s = vec3_cross(f, up);
    s = vec3_normalize(s);

    m_vec3 u = vec3_cross(s, f);
