#ifndef M_IO_FLT_MAT3_H
#define M_IO_FLT_MAT3_H
 ...
 */
O_INLINE
osize mat3_buffer(char *str, osize size, m_mat3 mat, bool colored, bool typed, bool multiline)
{
    return matn_buffer(str, size, mat.v, 3, colored, typed, multiline);
}
 ...
 * @param n the size (rows=n, cols=n)
 * @param colored true to use color codes for a terminal
 * @param typed if true, prints with the vec<n> type (3..4)
 * @param multiline if true, prints with newlines, else in a single line
 * @return characters written
 */
O_INLINE
char *mat3_str(oobj parent, m_mat3 mat, bool colored, bool typed, bool multiline)
{
    return matn_str(parent, mat.v, 3, colored, typed, multiline);
}
...
/**
 * Calls mat3_buffer and prints to stdout with colors (typed)
 * In the style:
 ...
 */
O_INLINE
osize mat3_print(m_mat3 mat, bool multiline)
{
    return matn_print(mat.v, 3, multiline);
}
 ...
 */
O_INLINE
osize mat3_println(m_mat3 mat, bool multiline)
{
    return matn_println(mat.v, 3, multiline);
}
 ...
 */
O_INLINE
bool mat3_json_read(m_mat3 *out_mat, oobj json_array)
{
    return matn_json_read(out_mat->v, 3, json_array);
}
 ...
 */
O_INLINE
oobj mat3_json_new(oobj parent, const char *name, m_mat3 mat)
{
    return matn_json_new(parent, name, mat.v, 3);
}

#endif //M_IO_FLT_MAT3_H
